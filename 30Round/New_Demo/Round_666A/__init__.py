...
class PWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if player6.trade:
            player6.payoff = (int(participation_count - 1) * ((int(C.TradeValue)) + int(C.TradeBonus))) + (int(6 - participation_count) * int(C.NullTrade))
        else:
...
page_sequence = [Waiting, chat, Participate, PWaitPage, Vote, ResultsWaitPage, Results]